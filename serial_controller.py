...
async def queue_use(serial_handler, logger, on_subscribe):
    message = await serial_handler.get_message()  # With caching
    if message is None:
        return
    if "debug" in os.environ:
        print("Swarm -> " + message)
    ...
    on_subscribe(port_name, value)

async def input_loop(serial_handler, logger, on_subscribe):
    while True:
        await queue_use(serial_handler, logger, on_subscribe)
